...
if strcmp (app.TrajectoriesPlotSwitch.Value, 'On') == 1
    try
        ...
        if strcmp(app.OrientationDropDown.Value,'Dynamic: GPS + IMU') == true
            try
                cd_var = interp1(linspace(min(app.refValue,[],'omitnan'),prctile(app.refValue,99.99),length(cd_var)),cd_var,app.refValue); % map color to velocity values
            catch
            ...
            cd_var = interp1(linspace(min(app.refValue,[],'omitnan'),prctile(app.refValue,99),length(cd_var)),cd_var,app.refValue); % map color to velocity values
        end
        cd_var = uint8(cd_var'*255); % need a 4xN uint8 array
        ...
        if quiver1 == 0
            ...
            X = xyzA_final2(ind1,1);
            Y = xyzA_final2(ind1,2);
            U = app.adjustedVel(ind1,1);
            V = app.adjustedVel(ind1,2);
            idx = ~isnan(U) & ~isnan(V);
            X = X(idx);
            Y = Y(idx);
            U = U(idx);
            ...
            colormap_choice = parula(256);
            color_indices = round(magnitude_norm * (size(colormap_choice, 1) - 1)) + 1;
            arrow_colors = colormap_choice(color_indices, :);

            % Plot the quiver with colored arrows
            for i = 1:numel(X)
                quiver(X(i), Y(i), U(i), V(i),15,...
