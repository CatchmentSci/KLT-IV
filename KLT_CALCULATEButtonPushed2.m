function KLT_CALCULATEButtonPushed2(app, ~)
            ...
            % interpolate the coordinates and depths at 0.01m distances - new in v1.1
            if max(diff(crossSectionIn(:,1))) == 0 % if monotonical in x - 1d interp
                interp_xs(1:round(total_length.*100)+1,1) = nanmin(crossSectionIn(:,1));
                interp_xs(1:round(total_length.*100)+1,2) = transpose(nanmin(crossSectionIn(:,2)):0.01:nanmin(crossSectionIn(:,2))+total_length);
                Vq = interp1(crossSectionIn(:,2),depthIn,interp_xs(:,2));
            elseif max(diff(crossSectionIn(:,2))) == 0 % if monotonical in y - 1d interp
                interp_xs(1:round(total_length.*100)+1,1) = transpose(nanmin(crossSectionIn(:,1)):0.01:nanmin(crossSectionIn(:,2))+total_length);
                interp_xs(1:round(total_length.*100)+1,2) = nanmin(crossSectionIn(:,2));
                Vq = interp1(crossSectionIn(:,1),depthIn,interp_xs(:,2));
            else % data changes in x and y directions
                total_length = arclength(crossSectionIn(:,1),crossSectionIn(:,2),'linear');
                interp_xs = interparc(0:(0.01/total_length):1,crossSectionIn(:,1),crossSectionIn(:,2),'linear');
                [YY,ZZ] = meshgrid(unique(crossSectionIn(:,1)),unique(crossSectionIn(:,2)));
                TQ = griddata(crossSectionIn(:,1),crossSectionIn(:,2),depthIn,YY,ZZ);
                Vq=interp2(YY,ZZ,TQ,interp_xs(:,1),interp_xs(:,2));
            end
